...
% Project the points in an (M-1)-d manifold to an (M-1)-d unit hypercube
    ...
    [N,M] = size(x);
    x  = x - repmat((sum(x,2)-1)/M,1,M);  % construct unix simplex   Σxi =1 ,xi is ith objective function value
    PF = PF - repmat((sum(PF,2)-1)/M,1,M); % construct unix simplex  ΣPFi =1
    x  = x - repmat(min(PF,[],1),size(x,1),1);
    x  = x./repmat(sum(x,2),1,M);
    x  = max(1e-6,x);   % none zero
    y  = zeros(N,M-1);
    for i = 1 : N
        c = ones(1,M);
        k = find(x(i,:)~=0,1);
